# rusty_h264-decoder — hardening audit
...
**Unit**: `crates/rusty_h264-decoder` — library (H.264 decoder)
**Tier**: critical-path — THE untrusted-input surface: it parses attacker-controlled H.264 bitstreams end to end
**Mirrors**: the crates.io page for `rusty_h264-decoder` (re-rendered at the next publish -- its link must be absolute) — every one of these carries the generated block and **must be
...
| H-25 | `cargo careful test` green | Completed | `cargo +nightly careful test -p rusty_h264-decoder --lib` run 2026-09-07: 12 passed, 0 failed, exit 0. | |
