...
| H-18 | ★ No `unwrap`/`expect`/panic on untrusted paths; typed errors | Completed | **47 production `.unwrap()`/`.expect()` sites -> 0** (commit 8335d92, 2026-09-07). None was ever on the bitstream parse path: 27 were lock poisoning, 2 condvar waits, 11 worker-thread channels and 7 internal invariants. Locks now use poison-tolerant accessors, channel failure is recorded and surfaced as the new typed `DecodeError::Internal`, and two sites were removed structurally. `decode()` returns `Result<_, DecodeError>` throughout. Re-counted 0 after the change; gated 68/68 byte-identical. | |
...
|---|---|---|---|---|---|
| 2026-09-07 | deep | Claude (use-protection-please v1, depth=deep) | 5 / 0 / 29 | 2 of 15 | First pass. Zero `Scheduled` by design: scheduling needs an owner and a date, which are the human's to assign, not the auditor's. |
|---|---|---|---|---|---|
