# rusty_h264-encoder — hardening audit
...
**Unit**: `crates/rusty_h264-encoder` — library (H.264 encoder)
**Tier**: standard — its input is caller-supplied YUV frames, not untrusted bytes; it produces bitstreams rather than consuming them
**Mirrors**: the crates.io page for `rusty_h264-encoder` (re-rendered at the next publish -- its link must be absolute) — every one of these carries the generated block and **must be
...
| H-18 | ★ No `unwrap`/`expect`/panic on untrusted paths; typed errors | Completed | TIER-SCOPED, and the scoping is the standard's own 'tier down, don't fake up' rule. H-18's criterion is 'no unwrap/expect/panic on paths reachable from UNTRUSTED INPUT'; this crate's input is the caller's own YUV frames, so it is `standard` tier and the denial that binds the decoder does not bind here. `unwrap_used`/`expect_used`/`panic` are therefore `allow` in its manifest WITH that reason written inline, rather than a `deny` that fails the build and gets deleted. The crates that do eat hostile bytes keep `deny`. | |
