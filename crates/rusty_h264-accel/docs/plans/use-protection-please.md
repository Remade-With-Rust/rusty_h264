...
| H-16 | ★ `unsafe` isolated, SAFETY-commented, inventoried | Incomplete | This is the crate the isolation exists for, and it is deliberately NOT `forbid(unsafe_code)` (documented in `src/lib.rs`). 161 `unsafe fn` plus 75 `unsafe {` blocks against 64 `// SAFETY:` comments -- roughly 85% of blocks commented, the rest not. No `UNSAFE.md` inventory with purpose and last-audit date. `cargo geiger`: 160/160 fns, 5609/5611 expressions. | |
...
| H-19 | Input validation — external bytes treated as hostile | Incomplete | Kernels take pre-validated lengths from the decoder rather than raw bytes, but that trust boundary is not written down anywhere -- which is exactly what makes an `unsafe` kernel crate's input contract auditable. | |
...
| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-26 | ★ Fuzz target per public parser, decoder, or message handler | Incomplete | See per-unit evidence. | |
...
| H-29 | Mutation and/or differential testing on critical modules | Incomplete | Kernels are pinned to scalar twins by `*_matches_scalar` oracles (e.g. the 50,000-round `mb_uniform_sse2_matches_avx2`), which is a differential test in the small. No mutation score recorded. | |
...
|---|---|---|---|---|---|
| 2026-09-07 | deep | Claude (use-protection-please v1, depth=deep) | 1 / 0 / 33 | 0 of 15 | First pass. Zero `Scheduled` by design: scheduling needs an owner and a date, which are the human's to assign, not the auditor's. |
|---|---|---|---|---|---|
