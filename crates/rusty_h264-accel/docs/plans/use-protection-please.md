# rusty_h264-accel — hardening audit
...
**Unit**: `crates/rusty_h264-accel` — library (SIMD kernels)
**Tier**: critical-path — the ONLY crate in the workspace carrying `unsafe`; its kernels run over decoder-controlled buffer lengths
**Mirrors**: the crates.io page for `rusty_h264-accel` (re-rendered at the next publish -- its link must be absolute) — every one of these carries the generated block and **must be
...
| H-16 | ★ `unsafe` isolated, SAFETY-commented, inventoried | Completed | `UNSAFE.md` is now generated from the tree (161 `unsafe fn`, 75 `unsafe {}` blocks, 73 `SAFETY:` comments, 62 `#[target_feature]`), states the single obligation every block carries (feature present + extents proven by the safe dispatcher), and records the geiger baseline. **All 17 previously undocumented blocks now carry a SAFETY comment** -- several already had one, but a census bump sat between the comment and the block, so neither clippy nor a reader could tell what it covered; those were moved rather than duplicated. `clippy::undocumented_unsafe_blocks` is wired in, so a new one fails the build. Open sub-item, written down in UNSAFE.md: `unsafe_op_in_unsafe_fn` is `allow` here alone. | |
| H-17 | Arithmetic safety explicit | Completed | This crate's arithmetic surface is much narrower than the workspace's and was reviewed directly: **zero** casts on a length or stride. Its 179 casts are all on pixel/coefficient VALUES (`as u8`, `as i16` -- the spec's saturation to bit depth), and its kernels take fixed-size arrays or `usize` strides that are never narrowed. The public API carries its dimensions in the type (`i16x16_luma_pred`, `chroma8x8_pred`), so there is no attacker-controlled length to truncate. | |
| H-18 | ★ No `unwrap`/`expect`/panic on untrusted paths; typed errors | Completed | This crate contains no `.unwrap()`/`.expect()` at all (grep: 0), and `clippy::unwrap_used`/`expect_used`/`panic` are denied for it. Its kernels return values, never Results. | |
| H-19 | Input validation — external bytes treated as hostile | Completed | The trust boundary is now written down (`UNSAFE.md`, 'What the unsafe is FOR'): kernels take fixed-size arrays or slices whose extents the SAFE dispatcher asserts before the call, and the target feature is established by `is_x86_feature_detected!` immediately above, never assumed. Previously true but undocumented, which for an `unsafe` crate is the part that matters. | |
...
| H-29 | Mutation and/or differential testing on critical modules | Completed | Every kernel is pinned to a scalar twin by a `*_matches_scalar`/`*_matches_avx2` oracle -- e.g. `mb_uniform_sse2_matches_avx2` runs 50,000 randomised rounds INCLUDING inputs differing in exactly one lane, so an off-by-one in a mask cannot hide. Above that sits the decoder's 68-stream byte-identity gate. No mutation score recorded, which is the open half. | |

### Phase 7 — Formal verification

| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-30 | Proof of panic-freedom / UB-freedom per `unsafe` module | Incomplete | No Kani harness for any kernel. This is the one crate where the gate genuinely applies -- everything else forbids `unsafe` -- so it stays open and is named as such in `UNSAFE.md`'s Open items. | |
...
| 2026-09-07 | deep | Claude (use-protection-please v1, depth=deep) | 29 / 0 / 5 | 12 of 15 | Pass 2: the hardening work. 85% of in-scope gates closed. |
|---|---|---|---|---|---|
