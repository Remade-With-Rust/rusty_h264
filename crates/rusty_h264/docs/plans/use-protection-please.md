...
**Unit**: `crates/rusty_h264` — library (facade / public API)
**Tier**: standard — a re-export facade; it holds no parser and eats no bytes of its own
**Mirrors**: the crates.io page for `rusty_h264` (re-rendered at the next publish -- its link must be absolute) — every one of these carries the generated block and **must be
...
| H-02 | Threat model revisited after last major change | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
...
| H-05 | ★ Release profile hardened (overflow-checks, LTO, panic policy) | Completed | `[profile.release]`: `overflow-checks = true` and an explicit `panic = "unwind"` (chosen, not defaulted -- the EDC worker's `catch_unwind` cleanup needs unwinding). Verified by running the FULL suite and the 68-stream byte-identity gate with overflow checks ON: all green, so nothing was relying on silent wraparound. | |
...
| H-10 | ★ `cargo vet` coverage complete | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
| H-11 | Unsafe inventory measured and trending down (geiger) | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
...
| H-16 | ★ `unsafe` isolated, SAFETY-commented, inventoried | Completed | `#![forbid(unsafe_code)]` -- enforced by `[workspace.lints.rust] unsafe_code = "forbid"`, which this crate inherits. There is no `unsafe` block to comment or inventory, so the gate is met by construction rather than by discipline. | |
...
| H-18 | ★ No `unwrap`/`expect`/panic on untrusted paths; typed errors | Completed | **47 production `.unwrap()`/`.expect()` sites -> 0** (commit 8335d92). None was on the bitstream parse path: 27 lock poisoning, 2 condvar waits, 11 worker channels, 7 internal invariants. Locks use poison-tolerant accessors; channel failure is recorded and surfaced as the new typed `DecodeError::Internal` at picture finalize (wrong pixels are never returned as if right); two sites were removed structurally. Re-counted 0. **Now mechanically enforced**: `clippy::unwrap_used`/`expect_used`/`panic` are DENIED for this crate and the build is clean under `-D warnings`, so it cannot regress. | |
| H-19 | Input validation — external bytes treated as hostile | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
| H-20 | ★ Secrets zeroized; never logged | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
| H-21 | Concurrency discipline | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |

### Phase 4 — Static analysis

| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-22 | Static analysis beyond the default linter runs on every PR | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
...
| H-24 | Critical paths pass the sanitizers (ASan/MSan/TSan) | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
| H-25 | `cargo careful test` green | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |

### Phase 6 — Fuzzing and properties

| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-26 | ★ Fuzz target per public parser, decoder, or message handler | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
| H-27 | ★ Continuous fuzzing with no open crashes | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
...
| H-29 | Mutation and/or differential testing on critical modules | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |

### Phase 7 — Formal verification

| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-30 | Proof of panic-freedom / UB-freedom per `unsafe` module | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
...
| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-34 | Vetted crypto only; no bespoke primitives | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
| H-35 | Side-channel discipline (constant-time, no secret branches) | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
| H-36 | Post-quantum migration plan for long-lived keys | N/A | Out of tier: gate applies to critical-path units only; this unit is `standard`. | |
...
|---|---|---|---|---|---|
| 2026-09-07 | deep | Claude (use-protection-please v1, depth=deep) | 2 / 0 / 32 | 0 of 12 | Pass 1: measured only. |
| 2026-09-07 | deep | Claude (use-protection-please v1, depth=deep) | 21 / 0 / 1 | 11 of 12 | Pass 2: the hardening work. 95% of in-scope gates closed. |
|---|---|---|---|---|---|
