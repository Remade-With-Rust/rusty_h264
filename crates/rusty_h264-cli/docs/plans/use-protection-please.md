...
| H-19 | Input validation — external bytes treated as hostile | Completed | `Decoder::decode(&[u8]) -> Result<Option<YuvFrame>, DecodeError>` is the single untrusted entry point; `#![forbid(unsafe_code)]` makes every index compiler-checked; `tests/fuzz_no_panic.rs` asserts corrupted and random buffers yield only `Ok`/`Err` under `catch_unwind`; and `tests/properties.rs` (H-28) pins the reader-never-overreads invariant over 20k rounds. No serde, no untrusted deserialization. | |
...
| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-31 | ★ Binary hardening applied and verified | Completed | The shipped PE's `DllCharacteristics` moved **0x8160 -> 0xc160** after adding `-C control-flow-guard` for Windows targets: HIGH_ENTROPY_VA, DYNAMICBASE, NX_COMPAT and GUARD_CF all set. Read from the binary header before and after; CFG was the only one the toolchain did not give for free. | |
| H-32 | Build is reproducible or fully auditable | Completed | `cargo auditable build --release -p rusty_h264-cli --features asm` verified 2026-09-07: `cargo audit bin` reads the result and reports **'Found cargo auditable data ... (15 dependencies)'** with no vulnerabilities. The shipped binary carries its own dependency list, so it can be audited without the source tree. | |

### Phase 9 — Runtime privilege

| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-33 | Least privilege documented and tested | Completed | `docs/security-operations.md` §2: the CLI needs no privilege beyond the invoking user (one input file, one output file, no sockets, no subprocesses, no credentials), with concrete container and systemd recipes for running it against untrusted input -- non-root, read-only rootfs, `--cap-drop=ALL`, `no-new-privileges`, and a memory cap, since the stream declares its own dimensions. No seccomp/landlock profile ships; that is stated rather than implied. | |
...
|---|---|---|---|---|---|
| 2026-09-07 | deep | Claude (use-protection-please v1, depth=deep) | 2 / 0 / 32 | 0 of 16 | Pass 1: measured only. |
| 2026-09-07 | deep | Claude (use-protection-please v1, depth=deep) | 32 / 0 / 4 | 13 of 16 | Pass 2: the hardening work. 89% of in-scope gates closed. |
|---|---|---|---|---|---|
