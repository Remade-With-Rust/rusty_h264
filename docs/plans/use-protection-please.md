# rs_h264 (workspace) — hardening audit
...
**Unit**: `.` — Cargo workspace root: shared config, CI, release process
**Tier**: critical-path — the repo root of an H.264 codec; it owns the toolchain, supply-chain and release gates for six shipped crates that eat untrusted bitstreams
...
| H-16 | ★ `unsafe` isolated, SAFETY-commented, inventoried | Completed | Enforced at the workspace level: `[workspace.lints.rust] unsafe_code = "forbid"` is the DEFAULT, and exactly two manifests opt out with a written reason (`-accel`, the SIMD crate; `-memprobe`, a test-only allocator probe). 'Which code may use unsafe' is a two-line grep. `clippy::undocumented_unsafe_blocks` is wired in, and the inventory lives at `crates/rusty_h264-accel/UNSAFE.md`. | |
...
| H-18 | ★ No `unwrap`/`expect`/panic on untrusted paths; typed errors | Completed | Enforced workspace-wide by policy: `clippy::unwrap_used`/`expect_used`/`panic` are DENIED, allowed only on the encoder (standard tier -- its input is the caller's own frames) and on dev/test targets where a panic IS the diagnostic. `-D warnings` is clean, and the policy caught two real `unwrap()`s in `-common` that pass 1 had missed. | |
...
| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-30 | Proof of panic-freedom / UB-freedom per `unsafe` module | Incomplete | No Kani/Creusot/Verus harness anywhere. Applies to `-accel` only, since every other crate forbids `unsafe`; the argument there today is the scalar-twin oracle plus 68-stream byte-identity, which is evidence, not verification. | |

### Phase 8 — Build and binary

| ID | Gate | Status | Evidence | Target |
|---|---|---|---|---|
| H-31 | ★ Binary hardening applied and verified | Completed | The shipped PE's `DllCharacteristics` moved **0x8160 -> 0xc160** after adding `-C control-flow-guard`: HIGH_ENTROPY_VA, DYNAMICBASE (ASLR), NX_COMPAT (DEP) and GUARD_CF (CFG) are now all set. Read directly from the binary header before and after. | |
...
| 2026-09-07 | deep | Claude (use-protection-please v1, depth=deep) | 30 / 0 / 5 | 13 of 16 | Pass 2: the hardening work. 86% of in-scope gates closed. |
|---|---|---|---|---|---|
