...
(0.14.0 + entropy rounds 1-6 + SIMD census + kernel + routing +
dense-over-scatter), plain-default build (`asm`, no knobs) run from snapshot
copies. Corpus regenerated by x264 and every stream byte-identical to
ffmpeg before timing; 1800-frame work parity both arms. ⚠ Quiet-box
precondition NOT met (a foreign `mistralrs` server held the box all
session), so absolute times are inflated on BOTH arms — read the ratio.

| tool tier | rusty CPU | ffmpeg CPU | rusty/ffmpeg | pairs | z    | rusty Mpx/s | ffmpeg Mpx/s |
| --------- | --------- | ---------- | ------------ | ----- | ---- | ----------- | ------------ |
| CAVLC     | 9,438 ms  | 5,688 ms   | 1.673x       | 7/7   | 2.65 | 176         | 292          |
| Main      | 12,391 ms | 7,672 ms   | 1.652x       | 7/7   | 2.65 | 134         | 216          |
| High      | 13,578 ms | 8,156 ms   | 1.673x       | 7/7   | 2.65 | 122         | 203          |

Against the 08-22 quiet-box record (1.98 / 2.16 / 2.06) and the 08-27 loaded
run below (2.06 / 1.99 / 1.96): every tier is now under 1.7x, and the tier
order has flattened — CAVLC, which was the most load-sensitive and the
furthest behind, is level with High. The rounds that moved it, in clock
order: routing (crowd +7.4%, all-intra +2.4%, shields CAVLC +6.9% vs round
6), dense-over-scatter (+1.8% / +1.1% on top), kernel round, SIMD
