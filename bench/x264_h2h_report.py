...
def ssim_db(s):
    """SSIM -> dB-like scale, so BD integrates it the way it integrates PSNR."""
    return -10.0 * math.log10(max(1.0 - s, 1e-9))


def polyfit3(xs, ys):
    """Degree-3 least squares via normal equations — mirrors bdrate.rs."""
...
def bd_rate(anchor, test):
    ...
    Negative = `test` needs FEWER bits for the same quality = test wins.
    """
    def prep(pts):
    ...
    da, la = prep(anchor)
    dt, lt = prep(test)
    if len(da) < 4 or len(dt) < 4:
        return None
    ca, ct = polyfit3(da, la), polyfit3(dt, lt)
    lo, hi = max(da[0], dt[0]), min(da[-1], dt[-1])
    if hi <= lo:
        return None  # curves do not overlap in quality; BD is undefined
    def integ(c, x):
