### In The Wild with 71,780 Active Installs
...
**Security** — this is a decoder, so it eats bytes an attacker chooses. The
[threat model](docs/threat-model.md) says what that means here, and
[SECURITY.md](SECURITY.md) is how to report something. The short version: the
decoder is `#![forbid(unsafe_code)]` with no production `unwrap`, all `unsafe`
lives in one SIMD crate ([inventory](crates/rusty_h264-accel/UNSAFE.md)), and
every release is gated on 68 streams decoding byte-identical to ffmpeg. Per-crate
hardening status is at the foot of each README.
...
**Tier** critical-path · **Audited** 2026-09-07 (deep) · **v1.0.0 gates** 12/16 · [Full checklist](docs/plans/use-protection-please.md)

`████████████████░░░░` **83%** &nbsp;·&nbsp; 29 Completed · 0 Scheduled · 6 Incomplete · 20 N/A
